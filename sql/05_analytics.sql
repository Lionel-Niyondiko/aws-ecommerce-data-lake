...
-- ###########################################################################
-- The data spans 2026-04-14 to 2026-07-13: exactly 91 days. A rolling
-- three-month window therefore covers 100% of it. The filter is correct and
-- reproducible, but it removes nothing here — say so before showing the number.
--
-- Alternative reading, three COMPLETE calendar months (May, June, July):
-- 6,146 rows, $7,638,493.36. It drops April, which is partial.
WITH bounds AS (
...
-- ###########################################################################
-- Q2 — Top 10 products by revenue and by quantity. The same ten?
-- ###########################################################################
-- Answer: no — 7 of 10 appear in both.
--
-- product_id = -1 is excluded. Without that filter "Unknown product"
-- ($183,284) would rank THIRD by revenue: a catch-all presented as a
-- best-seller.
SELECT p.product_id, p.title, p.category,
       ROUND(SUM(f.line_amount), 2) AS revenue,
       SUM(f.quantity)              AS quantity,
       ROUND(AVG(f.unit_price), 2)  AS avg_price   -- AVG, never SUM: not additive
FROM fact_ventes f
...
-- ###########################################################################
-- April and July are PARTIAL months (data starts on the 14th, ends on the
-- 13th). Their apparent drop is an extraction artefact, not a commercial
-- trend. Saying so is part of the answer.
--
-- Both order definitions are shown side by side so the bias is visible.
SELECT
...
--
-- Real trend, complete months only: May to June revenue +3.0%, average basket
-- +6.9%, on FEWER orders. Customers order less often but bigger.

-- Additivity proof. Expected: 7439 = 7439 (coherent), 4986 vs 2214 (not).
...
--
-- Verified: none of the 7,439 orders spans two countries, so grouping by
-- (invoiceno, date_id, country) cannot double count.
WITH orders AS (
...
--
-- The finding worth reporting: Switzerland is SECOND by revenue but LAST by
-- basket. It earns through order volume, not order value. An upsell lever
-- would beat an acquisition campaign there. Caveat: the spread between
-- extremes is only 14%, so this is worth watching, not restructuring around.


-- ###########################################################################
-- Q5 — Top 5 customers by cumulative revenue
-- ###########################################################################
-- customer_id < 0 excluded. Without that filter "Not recorded" (-2,
...
--
-- Coherence check: revenue attached to identified customers is 9,003,608.85,
-- 96.97% of the total. The remaining 3.03% is question 6.
...
--
--   exclude   revenue wrong by -5.0%, silently, no trace
--   isolate   revenue right, but the model fragments into two fact tables
--   attach    revenue right, traceable, model stays uniform   <- chosen
--
-- The deciding property: SUM(line_amount) returns true revenue whether or not
-- the dimensions are joined. An analyst unaware of the orphans is not wrong;
-- one who wants to study them writes WHERE product_id = -1.


-- ###########################################################################
-- Optional extensions
-- ###########################################################################
-- Revenue by product category, with monthly split.
-- kitchen-accessories 2,018,926.02 (21.74%) | groceries 1,816,924.54 (19.57%)
-- Two categories carry 41% of revenue — mostly a reflection of catalog
-- composition (30 and 27 products of 130), not of customer preference.
SELECT p.category,
       COUNT(*)                                            AS rows,
       ROUND(SUM(f.line_amount), 2)                        AS revenue,
       ROUND(100.0 * SUM(f.line_amount)
...
-- Return rate by category. Overall: 3.10% of rows, 3.42% of gross revenue.
--
-- STATISTICAL CAVEAT worth stating unprompted: mens-shoes shows 4 returns out
-- of 289 rows. Two more returns move the rate by 0.7 points. These rankings
-- are NOT significant and should not drive a decision. Several quarters of
-- data would be needed.
SELECT p.category,
       COUNT(*)                                                  AS rows,
       COUNT_IF(f.is_return)                                     AS returns,
       ROUND(100.0 * COUNT_IF(f.is_return) / COUNT(*), 2)        AS return_rate_rows,
       ROUND(SUM(CASE WHEN NOT f.is_return THEN f.line_amount END), 2) AS gross_revenue,
       ROUND(SUM(CASE WHEN f.is_return THEN f.line_amount END), 2)     AS returns_amount
FROM fact_ventes f
JOIN dim_produit p ON p.product_id = f.product_id
GROUP BY p.category
ORDER BY return_rate_rows DESC;
